...
#[tonic::async_trait]
impl BessControl for BESSControlService {
    ...
    async fn pause_all(
        ...
        //     pause_all_workers();
        info!("*** All workers have been paused ***");
        //     return Status::OK;
        Ok(Response::new(EmptyResponse {
            error: Some(bess_pb::Error {
                code: 0,
    ...
    async fn list_workers(
        ...
        for wid in 0..worker::K_MAX_WORKERS {
            ...
            let mut status: Vec<list_workers_response::WorkerStatus> = Vec::new();

            status.push(list_workers_response::WorkerStatus {
                wid: wid as i64,
                core: (),
                running: worker::is_worker_running(wid),
                num_tcs: (),
                silent_drops: (),
            });
    ...
    async fn kill_bess(
        ...
        //     WorkerPauser wp;
        warn!("Halt requested by a client\n");
    ...
    async fn import_plugin(
        &self,
        request: Request<ImportPluginRequest>,
    ) -> Result<Response<EmptyResponse>, Status> {
        //     std::lock_guard<std::recursive_mutex> lock(mutex_);
        ...
        if !bessd::load_plugin(path) {
            // return return_with_error(response, -1, "Failed loading plugin %s",
            //                          request->path().c_str());
            let status = Status::new(Code::Aborted, format!("Failed loading plugin {}", path));
            return Err(status);
        }
        //     return Status::OK;
        Ok(Response::new(EmptyResponse {
